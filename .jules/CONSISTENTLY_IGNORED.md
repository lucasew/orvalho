...
**- Justification:** Source code comments must document the "why" for developers, not the AI's internal reasoning or conversational filler.
...
**- Pattern:** Using major version tags like `@v4` for `actions/checkout` or `@v2` for `mise-action` instead of explicitly pinning exact versions, or downgrading them explicitly against instructions.
