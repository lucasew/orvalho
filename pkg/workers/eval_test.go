...
func TestAsyncFunctionCJSFunctionDefault(t *testing.T) {
	iso := New("", Options{
		Imports: importMap(map[string]any{
			"autoprefixer": imports.Script{Source: `module.exports = function plugin() { return { postcssPlugin: "autoprefixer" }; };`},
		}),
	})
	err := iso.ScriptMain(t.Context(), `
		var AsyncFunction = async function () {}.constructor;
		...
		var fn = new AsyncFunction(
			...
			"__vite_ssr_exportAll__",
			"__vite_ssr_exportName__",
			"\"use strict\";\nimport plugin from \"autoprefixer\";\nexport default plugin();\n"
		);
		var n = 0;
		Promise.resolve(fn(exports, { url: "file:///t.mjs" }, null, null, null, exportName)).then(function () {
			if (!exports.default || exports.default.postcssPlugin !== "autoprefixer") throw new Error("plugin " + exports.default);
			n = 1;
		});
		setTimeout(function () {
			if (n !== 1) throw new Error("no result");
...
func TestAsyncFunctionPostcssConfigObject(t *testing.T) {
	iso := New("", Options{Imports: NodeScriptImports()})
	err := iso.ScriptMain(t.Context(), `
		var AsyncFunction = async function () {}.constructor;
		var exports = {};
		...
		var n = 0;
		Promise.resolve(fn(exports, { url: "file:///postcss.config.js" }, null, null, null, exportName)).then(function () {
			if (!exports.default || !exports.default.plugins) throw new Error("cfg " + exports.default);
			n = 1;
		});
		setTimeout(function () {
			if (n !== 1) throw new Error("no result");
