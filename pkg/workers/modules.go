...
func (iso *Isolate) loadScript(key, source, file string) (goja.Value, error) {
	...
	if isESMLexerFile(file) {
		final = iso.patchESMLexer(final)
		_ = module.Set("exports", final)
	}
...
// importMethodDef is true for `{ import(src) { ... } }` so we do not
// rewrite the method name to __import (Astro's Vite loader.import).
